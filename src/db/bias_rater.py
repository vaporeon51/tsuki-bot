...
def record_vote(user_id: int, winner_id: str, loser_id: str) -> Tuple[int, int, int, int]:
    """
    Records a vote and updates global and personal ELO.
    Returns (global_winner_delta, global_loser_delta, personal_winner_delta, personal_loser_delta).
    """
    with psycopg.connect(**CONN_DICT) as conn:
        with conn.cursor() as cur:
            # 1. Ensure user_elo entries exist (default 1200)
            cur.execute(
                """
            ...
            # 2. Get current global and personal ELOs
            cur.execute(
            ...
            cur.execute(
                "SELECT role_id, personal_elo FROM user_elo WHERE user_id = %s AND role_id IN (%s, %s);",
            ...
            cur.execute(
                """
                UPDATE user_elo SET personal_elo = personal_elo + %s WHERE user_id = %s AND role_id = %s;
                """,
            ...
            cur.execute(
                ...
                (user_id, winner_id, loser_id, global_winner_delta, global_loser_delta, personal_winner_delta, personal_loser_delta)
                VALUES (%s, %s, %s, %s, %s, %s, %s);
                """,
                (user_id, winner_id, loser_id, gw_delta, gl_delta, pw_delta, pl_delta),
            )

            return gw_delta, gl_delta, pw_delta, pl_delta
