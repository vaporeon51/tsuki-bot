...
@discord.app_commands.describe(
    query="Idols and groups to include; ignored when idol_pool is not All",
...
async def autofeed(
    ...
    task = asyncio.create_task(autofeed_command(interaction, query, idol_pool, interval, count, sort_by))
...
async def autofeed_command(
    ...
    if idol_pool == "all":
        ...
        if not tops.entries:
            text = (
                f"i couldn't find any {idol_pool} bias rankings yet, "
                f"try `/bias vote` first {HANNI_EMOJIS['thinking']}"
            )
            print(text)
            message = await interaction.followup.send(content=text, wait=True)
            await message.delete(delay=30)
    ...
    if idol_pool == "all":
        ...
        text = feed_started_message("bias", sort_by, len(role_ids_and_urls), interval, idol_pool=idol_pool)
